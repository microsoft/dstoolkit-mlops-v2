...
Let's say our data scientist's dedicated directory is `experimentation` placed at the root of the `dstoolkit-mlops-v2`.  Using the sample `London Taxi CI Workflow`(`.github/workflows/london_taxi_ci_pipeline.yml`) in the [dstoolkit-mlops-v2](https://github.com/microsoft/dstoolkit-mlops-v2), we could configure GitHub workflows to ignore this directory by adding a `paths-ignore` block.  For example:
